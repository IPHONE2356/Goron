module.exports = {
	name:"deafen",
	description: "Deafens the specified user using theur mention tag - Only a number of people have access to this command",
	usage:"@targetsmentiontag",
	execute(message,args){
		...
		if(counter < 3){
			...
			if(!args.length){
				if(superauth == true){
					message.channel.send("Please specify a target to deafen, Correct Usage: ``!deafen @Stinkybreathjaimin``")
				}
					...
					if(message.author.id != superuser[0] && target == superuser[0]){
						...
						members.forEach(member => {
							if(member.id == imposter){
								...
								.setColor("00A6FF")
								.setTitle("Deafen")
								.setDescription(`Deafened ${member.nickname} :x: :microphone2: \n Because they thought they were smart`)
								message.channel.send(embed)
